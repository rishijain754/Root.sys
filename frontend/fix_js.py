...
with open('c:/Users/Rishi/Documents/GitHub/Root.sys/frontend/index.html', 'r', encoding='utf-8') as f:
    content = f.read()

# Find everything up to <script>
...
function addBotMessage(text, actions = []) {
    ...
    if (actions.length > 0) {
        ...
        <div class="chat-actions">
            ${actions.map(action => `
                <button class="chat-action" onclick="quickTopic('${action.replace(/'/g, "\\\\'")}')">${action}</button>
            `).join("")}
        </div>
        `;
    }
...
async function processMessage(text) {
    ...
    try {
        ...
        if (data.response_type === 'HUMAN_HANDOFF') {
             ...
             addBotMessage(data.message.replace(/\n/g, '<br>'));
...
function startAgentFromModal() {
    ...
    addBotMessage("Sure. Tell me what happened in your own words.\n\nIf you have a reference ID, include it as well.\n\nOnce I have the issue summary, I'll create a support ticket and provide your callback window.");
    input.focus();
}
...
with open('c:/Users/Rishi/Documents/GitHub/Root.sys/frontend/index.html', 'w', encoding='utf-8') as f:
